...
`--solve-question` 默认只用本地规则推理，不访问 LLM。当前规则会优先处理剧本人数计算题，再提取正文里的“答案/谜底/口令/暗号/密码”、评论/回复口令、简单选项提示和基础四则运算；没有命中时输出“不知道”。需要模型推理时显式使用 `--LLM`。

剧本人数计算题会读取本地 JSON 人数表。默认读取当前目录的 `script_player_counts.json`，也可以用 `EASYMONEY_SCRIPT_COUNTS_PATH` 指定路径；仓库里有 `script_player_counts.example.json` 可复制参考：
...
`aliases` 可用于题目里的简称或别名，例如题目写 `【暗夜】` 时会命中 `黎明系列：暗夜将至` 的人数。

人数题支持在总和后按题面顺序追加 `+`、`-`、`×`、`÷` 修正项，例如 `剧本【青白+草芥】玩家人数总和✖️2➕5🟰？`。
